...
//#define SILENT // activate verbose messaging for debugging

Adafruit_AM2315 am2315;
Adafruit_BMP280 bmp280;

SoftwareSerial gsmGprs(2, 3); 
//SoftwareSerial esp32WiFi(4, 5);

#define C_GSM_MODE  0
#define C_WIFI_MODE 1

#define C_DEFAULT_MODE C_GSM_MODE

const unsigned BMP_I2C_ADDR = 0x76; // bmp280
//const unsigned BMP_I2C_ADDR = 0x77 // bmp180

// server interface
const String BASE_URL               = "<your URL>";
...
String input_string = "";
int transmission_mode = C_DEFAULT_MODE;
...
void transmit_values(int transmission_mode, String server_url, String handler, String val) {
  ...
  #ifndef SILENT
    Serial.print("[Info] Transmiting data with mode : ");
    Serial.println(transmission_mode);
  #endif

  switch(transmission_mode){
    case C_WIFI_MODE :
      // send to the esp32 serial interface
      #ifndef SILENT
        Serial.print("[Info] WiFi mode selected : ");
        Serial.println(transmission_mode);
      #endif

      //esp32WiFi.println(val);
      break;
    ...
    case C_GSM_MODE:
      ...
      #ifndef SILENT
        Serial.print("[Info] GSM mode selected : ");
        Serial.println(transmission_mode);
      #endif
    ...
    default:
      ...
      #ifndef SILENT
        Serial.print("[Info] Serial (default) mode selected : ");
        Serial.println(transmission_mode);
      #endif
...
void setup() {
  ...
  // activate the ESP32 serial interface
  // esp32WiFi.begin(9600);
...
void loop() {
  ...
  while (esp32WiFi.available() > 0) {

    char incomingChar = (char)esp32WiFi.read();    
    if (incomingChar == '\n') { // command line finished           
      
      int cmd_tmp, arg_tmp;
      if( parseCmdArg(input_string, &cmd_tmp, &arg_tmp) ) {
        #ifndef SILENT
          Serial.println("[INFO] Command is valid");
        #endif
          cmd = cmd_tmp;
          arg = arg_tmp;
      } else {
        #ifndef SILENT
          Serial.println("[ERROR] Invalid command input");
        #endif
        cmd = 1;
        arg = 0;
      }
        ...
        case 3:
          switch(arg){
            ...
            case 1:
              ...
              break;
            default:
              transmission_mode = C_DEFAULT_MODE;
              break;
          }
          break;
      }
